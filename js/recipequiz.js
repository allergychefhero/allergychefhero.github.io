...
function getHeading(){
    ogheading=document.getElementById('quiz-heading').innerHTML;
}
...
function tryAgain(){
  ...
  document.getElementById('quiz-heading').innerHTML="I’m allergic to that.";
...
function quizProgression() {
  ...
  if(btnText == 'Start! &gt;'){
    window.location='ingredlist_'.concat(document.getElementById('name').className,'.html');
    }
  //if try again-- wrong choice
    ...
    if(choiceSelected){
      ...
      document.getElementById('quiz-heading').innerHTML=ogheading;
    }
  }
}
